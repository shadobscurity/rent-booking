...
export default async function Detail({ params, searchParams }:{ params: { slug: string }, searchParams: { wear_date?: string; extra_duration?: string; delivery_type?: string }}) {
  ...
  const { data: product } = await supabaseAdmin
    .from('products')
    .select('id, name, brand, description, base_price, extra_day, deposit, variants ( id, size ), slug')
    .eq('slug', params.slug).single()

  if (!product) return <div>Not found</div>

  const price = calcPrice(product.base_price as number, product.extra_day as number, extra)
      ...
      <div>
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="text-lg font-medium mt-2">
            ...
            <span className="block text-sm opacity-75">Deposit: Rp {(product.deposit as number).toLocaleString('id-ID')}</span>
          </div>
